...
This section will provide simple scatterplots and linear regression models. Before we start, let's create a plotting and analysis function. This function will return `ggplot` object and $R^2$ using simple linear model. 
...
Samozino model assumes that take-off velocity (TOV) is double mean velocity (MV) (it is simplification that allows for easier calculus from practical field data collection). But is that the case? Here is the scatterplot:
...
As can be seen, theoretical model (the one that uses *true* mean velocity) is not very good at predicting bdyweight jump height. Practical Samozino (or the original one), one the other hand is very good:
...
It is thus my recomendation to Samozino *et al.* to avoid using *mean velocity* in the profile, and rather stick to TOV instead. This way the unnecessary assumptions doesn't need to be applied and optimization procedure becomes less confusing.
...
Although high $R^2$, using SESOI, this model is not great for predicting max force. But it is indeed highly correlated. From the variable importance graph, it can be seen that `samozino_practical_profile.F0` is the most important variable, followed by bodyweight and push-off distance. This can be seen from the PDP+ICE plots as well (these are used to estimate the variable importance using algorythm explained in @greenwellSimpleEffectiveModelBased2018).
...
The final exploration involve max power ($P_{max}$). Let's check the simple scatterplots first:
...
# Aligment between true bodyweight jump height and model predictions
...
Relative $P_{max}$ shows very high association with jump height (this could be the *circular causality* explained in the previous vignette: we use performance metrics to device underlying determinants of performance). Let's perform multivatiate prediction usinig linear regression with interactions:
...
It seems that LPF slope give unperfect hints only for effects of improving max force, but nothing else. 
...
This simple model concludes that `samozino_practical_profile.Sfv_perc` can give us glimples into which intervention should be done (i.e., improving max force or max velocity). 
...
Unfortunatelly, LPF slope does not provide insight to which intervention should be done to improve the vertical jump height. 
...
It looks like Bosco Index has a similar relationship with max force incremens as LPF slope. Let's chech the relationship between the two:
...
- Cluster 2: explosiveness (how quick is the take-off, rate of force and power developelopment)
- Cluster 3: velocity 
