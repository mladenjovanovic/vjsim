...
5. Before deducting viscous force from generated force, viscous force needs to be *scaled down* using force percentage. In more complex simulation, viscous force would have it's own Force-Length characteristic, but in this simple version, we are going to assume it is the same as Force-Length charcteristic for the max force. If we do not scale viscous force, at the last moments before the take-off, due sudden drop of force percentage to 0, viscous force would be much higher than the generated force, which would yield negative ground reaction force (which is imposible for a jump). 
6. Ground reaction force (GRF) is the force acting on the load. GRF is the *most important force* in the `vjsim`, since it represent the foce usually tracked in the sport science.
