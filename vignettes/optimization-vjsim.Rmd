...
Similar idea is utilizied with *Bosco Index* where the ratio of 2x BW jump height to BW jump height is calculated. Unfortunately, neither of these two (Bosco Index nor LPF slope) gives us a practical summary such as Samozino profile (i.e. using $S_{FV}\%$ nor $probe_{IMB}$). Thus we can only assume what type of intervention should be done, but we can use them to compare between individuals. On the other hand, Samozino profile gives us a hint on which *quality* one needs to be working (and by quality I assume parts of the curve; i.e. F0 or V0) to get the biggest effect on bodyweight jump. 

In the next vignette I will continue with the discussion and exploration of these model by using simulated data for a large ammount of different combinations of Force Generator parameters. I will follow up with discussion of intervention studies using this model [@jimenez-reyesEffectivenessIndividualizedTraining2017; @jimenez-reyesOptimizedTrainingJumping2019] and provide final conclusion about it's application. 
